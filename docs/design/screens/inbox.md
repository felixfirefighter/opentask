...
- Inbox/list: schedule, priority, and tags.
...
| Partial (Upcoming) | When bounded recurrence expansion reaches a safety cap, label loaded groups incomplete and read-only, name that work may be missing, and offer Retry. A zero-row partial result is not an empty seven-day range. |
