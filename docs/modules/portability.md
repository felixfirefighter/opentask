...
- Exclude credentials, sessions, provider secrets, encrypted push material, queue internals, and server configuration.
...
- `exportUserData(actor)` opens a consistent read snapshot and composes authorized identity, task, habit, focus, notification, and assistant records.
...
- Export contains no other user's record and no server secret, password hash, account/session token, rate-limit state, VAPID/OpenAI key, encrypted push endpoint/key, pg-boss row, or internal provider payload.
- The notification section includes reminder definitions, nonsecret delivery state, and nonsecret subscription metadata; endpoint hashes and push credentials are excluded.
...
- Export endpoints are never service-worker cached and export logs contain metadata only, not content.

## Dependencies

- Authorized public export readers from identity, tasks, habits, focus, notifications, and assistant.
...
- Secret/redaction tests covering Better Auth, push, pg-boss, OpenAI, environment, and logs.
...
- HTTP authorization, content type, attachment filename, `Cache-Control: private, no-store`, and service-worker exclusion tests.
